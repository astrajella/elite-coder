...
def init_db():
    ...
    cur = conn.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS jobs (id INTEGER PRIMARY KEY, dataset_path TEXT, base_model TEXT, adapter TEXT, status TEXT, result TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    conn.commit(); conn.close()
init_db()

def queue_job(dataset_path, base_model, adapter='lora_adapter'):
    conn = sqlite3.connect(DB); cur = conn.cursor()
    cur.execute('INSERT INTO jobs(dataset_path,base_model,adapter,status) VALUES(?,?,?,?)', (dataset_path, base_model, adapter, 'queued'))
...
def worker_loop(poll_interval=2.0):
    while True:
        ...
        if row:
            ...
            cur.execute('UPDATE jobs SET status=? WHERE id=?', ('running', jid)); conn.commit()
            # simulate training
            time.sleep(2)
            result = {'status':'done','details':f'Trained {base} with {adapter} on {dataset}'}
            cur.execute('UPDATE jobs SET status=?, result=? WHERE id=?', ('done', json.dumps(result), jid)); conn.commit()
        conn.close()
