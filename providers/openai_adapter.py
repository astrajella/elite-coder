...
def generate(prompt, model=None, stream=False):
    ...
    if not ALLOW or httpx is None:
        return {'provider':'openai','model':model,'text':f'[SIMULATED OPENAI] {prompt[:200]}', 'stream': False}
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}"}
    payload = { "model": model, "messages": [{"role":"user","content":prompt}] }
    if stream:
        # streaming not implemented fully - fallback to non-stream
        pass
    r = httpx.post("https://api.openai.com/v1/chat/completions", json=payload, headers=headers, timeout=30.0)
    r.raise_for_status()
    return r.json()
