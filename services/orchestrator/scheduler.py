...
# --- Orchestrator concurrency & streaming accounting additions ---

from prometheus_client import Counter, Histogram, Gauge
from collections import defaultdict, deque
from . import queue_sqlite
from .queue_sqlite import lease_acquire, lease_renew, lease_release, requeue_orphans, execution_seen, execution_mark, step_set_trace
import threading
...
# Prometheus metrics
TOOL_CALLS = Counter('orch_tool_calls_total', 'Total tool calls', ['persona','tool','status'])
TOOL_TOKENS = Counter('orch_tool_tokens_total', 'Total tokens by tool', ['persona','tool'])
TOOL_COST = Counter('orch_tool_cost_total', 'Total cost by tool', ['persona','tool'])
TOOL_LATENCY = Histogram('orch_tool_call_seconds', 'Tool call latency seconds', ['persona','tool'])

# Percentile gauges (computed over sliding window in-memory)
P50 = Gauge('orch_tool_latency_p50_seconds', 'p50 latency (sliding window)', ['persona','tool'])
P95 = Gauge('orch_tool_latency_p95_seconds', 'p95 latency (sliding window)', ['persona','tool'])
P99 = Gauge('orch_tool_latency_p99_seconds', 'p99 latency (sliding window)', ['persona','tool'])
...
def init_tracer():
    ...
    if endpoint and OTLPHTTPSpanExporter:
        provider.add_span_processor(BatchSpanProcessor(OTLPHTTPSpanExporter(endpoint=endpoint)))
...
from . import queue_sqlite
from .queue_sqlite import lease_acquire, lease_renew, lease_release, requeue_orphans, execution_seen, execution_mark, step_set_trace, contextlib, itertools
from collections import defaultdict, deque
from . import queue_sqlite
from .queue_sqlite import lease_acquire, lease_renew, lease_release, requeue_orphans, execution_seen, execution_mark, step_set_trace
...
async def _worker_loop(worker_id: int):
    ...
    while True:
        ...
        if not item:
            await asyncio.sleep(0.2)
            continue
        run_id = item['run_id']; plan = item['plan']; project = item.get('project')
...
import os, math, asyncio, time, uuid, json, httpx
from . import store

AGENT_CORE = os.getenv('AGENT_CORE_URL', 'http://localhost:8001')
LEDGER_URL = os.getenv('LEDGER_SERVICE_URL', 'http://localhost:8003')
ORCH_SERVICE_TOKEN = os.getenv('ORCH_SERVICE_TOKEN', '')  # used when calling other services
ORCH_RETRY_MAX = int(os.getenv('ORCH_RETRY_MAX','3'))
ORCH_RETRY_BACKOFF = float(os.getenv('ORCH_RETRY_BACKOFF','2.0'))  # multiplier base seconds

async def call_tool(persona, tool, payload, timeout=60, idempotency_key=None, expected_schema=None, stream=False):
    tracer = init_tracer()
    from time import time as _now



# --- BEGIN auto-instrumentation: aiohttp.ClientSession.post wrapper (enhanced) ---
import aiohttp as _aiohttp_lib
_orig_aiohttp_post = _aiohttp_lib.ClientSession.post

def _redact_payload(obj, keys_to_redact=None):
    # recursively redact sensitive keys
    if keys_to_redact is None:
        keys_to_redact = ('key','token','secret','password','authorization','api_key','apiKey','access_token')
    try:
        if isinstance(obj, dict):
            out = {}
            for k,v in obj.items():
                kl = k.lower() if isinstance(k, str) else k
                if any(s in str(kl) for s in keys_to_redact):
                    out[k] = 'REDACTED'
                else:
                    out[k] = _redact_payload(v, keys_to_redact)
            return out
        elif isinstance(obj, (list,tuple)):
            return [_redact_payload(x, keys_to_redact) for x in obj]
        else:
            return obj
    except Exception:
        return 'REDACTED'

async def _aiohttp_post_wrapper(self, url, *args, **kwargs):
    import time as _time, os as _os, json as _json
    start = _time.time()
    tool = None
    persona = None
    try:
        # infer persona from JSON body or headers
        try:
            j = kwargs.get('json') or kwargs.get('data') or {}
            if isinstance(j, dict):
                persona = j.get('persona') or j.get('actor') or j.get('persona_id')
                if not persona and 'author' in j:
                    persona = j.get('author')
        except Exception:
            persona = None
        # infer persona from headers if not in json
        try:
            headers = kwargs.get('headers') or {}
            if not persona and isinstance(headers, dict):
                persona = headers.get('X-Persona') or headers.get('x-persona') or headers.get('Persona')
        except Exception:
            pass
        # determine tool name from URL path (last path segment)
        try:
            tool = str(url).rstrip('/').split('/')[-1]
            if not tool:
                tool = 'unknown'
        except Exception:
            tool = 'unknown'
        ...
        resp = await _orig_aiohttp_post(self, url, *args, **kwargs)

        status = getattr(resp, 'status', None) or getattr(resp, 'status_code', None) or 0
        # emit metric for success
        try:
            TOOL_CALLS.labels(persona or 'unknown', tool or 'unknown', 'success').inc()
        except Exception:
            pass

        # If a validation endpoint is configured and the request indicated an expected schema, call validator
        try:
            validate_base = _os.getenv('AGENT_CORE_VALIDATE_URL') or _os.getenv('AGENT_CORE_URL')
            expected_schema = None
            if isinstance(j, dict):
                expected_schema = j.get('schema_name') or j.get('expected_schema')
            if validate_base and expected_schema:
                validate_url = str(validate_base).rstrip('/') + '/tool_validate_schema'
                # redact payload before sending
                safe_body = _redact_payload(j)
                # Post to validation endpoint using original post to avoid recursion issues
                try:
                    # build kw only with json and timeout to avoid passing complex session objects
                    vresp = await _orig_aiohttp_post(self, validate_url, json={'schema_name': expected_schema, 'payload': safe_body}, timeout=5)
                    # don't rely on vresp content; it's best-effort
                except Exception:
                    pass
        except Exception:
            pass
        ...
        try:
            TOOL_CALLS.labels(persona or 'unknown', tool or 'unknown', 'failure').inc()
        except Exception:
            pass
        raise
    finally:
        try:
            ...
            TOOL_LATENCY.labels(persona or 'unknown', tool or 'unknown').observe(dur)
        except Exception:
            pass

# Patch only once
try:
    if _aiohttp_lib.ClientSession.post is not _aiohttp_post_wrapper:
        _aiohttp_lib.ClientSession.post = _aiohttp_post_wrapper
except Exception:
    pass
# --- END auto-instrumentation ---
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        try:
            ...
            raw = r.text
            # SSE blocks
            for payload in _iter_sse_lines(raw):
                try:
            ...
            if agg['chunks']>0:
                ...
                # try JSONL
                tokens=0; cost=0.0; chunks=0
                for line in raw.splitlines():
                    try:
                        obj=json.loads(line)
                        tokens += int(obj.get('tokens',0)); cost += float(obj.get('cost',0.0)); chunks+=1
                    except Exception:
                        pass
                if chunks>0:
                    j={'stream_summary':{'tokens':tokens,'cost':cost,'chunks':chunks}}
                else:
        ...
        if expected_schema:
            try:
                ...
                vres = vs.json() if vs.ok else {'valid': False, 'errors':[{'msg':'validate_schema call failed','status': getattr(vs,'status_code',None)}]}
        ...
        tokens_val = j.get('tokens', j.get('token_est', j.get('stream_summary',{}).get('tokens',0)))
        cost_val = j.get('cost', j.get('cost_est', j.get('stream_summary',{}).get('cost',0.0)))
        try:
...
from .schemas import ToolOutput, validate_tool_payload
from . import queue_sqlite
from .queue_sqlite import lease_acquire, lease_renew, lease_release, requeue_orphans, execution_seen, execution_mark, step_set_trace, time, uuid, os, json, httpx
from . import store

AGENT_CORE = os.getenv('AGENT_CORE_URL', 'http://localhost:8001')
LEDGER_URL = os.getenv('LEDGER_SERVICE_URL', 'http://localhost:8003')

async def call_tool(persona, tool, payload, timeout=60):
    # call agent-core tool endpoint; wrap timings and return result
    url = f"{AGENT_CORE}/tool_{tool}"
    # some older services may use different naming; also try /tools/{tool}
    async with httpx.AsyncClient(timeout=timeout) as client:
        with tracer.start_as_current_span(f"tool:{tool}") as span:
            span.set_attribute('persona', persona)
            span.set_attribute('tool', tool)
        ...
        start = time.time()
        try:
            r = await client.post(url, json={'persona':persona, 'payload': payload})
        except Exception as e:
            status_label='exception'
            with tracer.start_as_current_span('tool.error') as es:
                es.set_attribute('error', str(e))
        ...
        try:
            ...
            j = {'raw': r.text, 'status': r.status_code}
        # log to ledger if available
        try:
            await client.post(f"{LEDGER_URL}/log_run", json={
                'persona': persona,
            ...
            pass
        return {'ok': True, 'response': j, 'duration': duration, 'status_code': r.status_code if hasattr(r,'status_code') else None}
...
async def execute_plan(run_id, plan):
    ...
    if store.is_cancelled(run_id):
        trace['steps'].append({'step_id': step_id, 'description': desc, 'status': 'cancelled'})
    ...
    matched = next((s for s in existing_steps if s.get('step_id')==step_id and s.get('status')=='ok'), None)
    if matched:
    ...
    while attempt < ORCH_RETRY_MAX and not success:
        attempt += 1
        trace['steps'].append({'step_id': step_id, 'description': desc, 'status': 'running', 'attempt': attempt, 'started_at': time.time(), 'persona': persona, 'tool': tool})
        store.update_run_trace(run_id, 'running', trace)
        try:
            res = await call_tool(persona, tool, payload, idempotency_key=idempotency_key, expected_schema=expected_schema)
            trace['steps'][-1].update({'result': res, 'ended_at': time.time()})
            if res.get('ok'):
                ...
                trace['steps'][-1].update({'status':'error', 'error': last_err})
                store.update_run_trace(run_id, 'running', trace)
            ...
            last_err = str(e)
            trace['steps'][-1].update({'status':'error', 'error': last_err, 'ended_at': time.time()})
            store.update_run_trace(run_id, 'running', trace)
    ...
    if not success:
        ...
        step_id = step.get('step_id') or str(uuid.uuid4())
        desc = step.get('description','')
        # decide which tool/persona to call; simple mapping: coder->generate_code, critic->critic_review, summarizer->summarize_tokens, planner->plan_step, tester->run_tests, commit->commit_and_artifact
        persona = step.get('persona', 'coder')
        tool = step.get('tool') or step.get('action') or 'generate_code'
        payload = step.get('payload', {})
        # call tool
        store.update_run_trace(run_id, 'running', trace)
        trace['steps'].append({'step_id': step_id, 'description': desc, 'status': 'running', 'started_at': time.time()})
        store.update_run_trace(run_id, 'running', trace)
        try:
            res = await call_tool(persona, tool, payload)
            trace['steps'][-1].update({'status': 'ok' if res.get('ok') else 'error', 'result': res, 'ended_at': time.time()})
            store.update_run_trace(run_id, 'running', trace)
            if not res.get('ok'):
                # stop on error and mark failed
                store.update_run_trace(run_id, 'failed', trace)
                return {'ok': False, 'trace': trace}
        except Exception as e:
            trace['steps'][-1].update({'status': 'error', 'error': str(e), 'ended_at': time.time()})
            store.update_run_trace(run_id, 'failed', trace)
            return {'ok': False, 'trace': trace}
...
METRIC_RETRIES = Counter('orch_retries_total','Total retries', ['run_id','step_id'])
METRIC_VALIDATION_ERRORS = Counter('orch_validation_errors_total','Tool validation errors', ['tool','schema'])
METRIC_IDEMPOTENT_SKIPS = Counter('orch_idempotent_skips_total','Steps skipped because already done', ['run_id','step_id'])
...
ORCH_STEP_DURATION = Histogram("orch_step_duration_seconds", "Duration of a step execution", buckets=[0.05,0.1,0.25,0.5,1,2,5,10,30,60])
ORCH_LEASE_WAIT = Histogram("orch_lease_wait_seconds", "Time spent waiting to acquire a step lease", buckets=[0.001,0.01,0.05,0.1,0.25,0.5,1,2,5])


# Summaries for client-side quantiles (p50/p90/p99)
ORCH_STEP_DURATION_SUMMARY = Summary("orch_step_duration_seconds_summary", "Step duration summary")
ORCH_LEASE_WAIT_SUMMARY = Summary("orch_lease_wait_seconds_summary", "Lease wait summary")
