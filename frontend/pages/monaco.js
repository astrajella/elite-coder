...
export default function MonacoPage(){  
  ...
  async function commitPatches(){
    ...
    const res = await fetch('/api/agent/commit', {method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body: JSON.stringify({commit_message:'Improve add naming', patches})})
    const j = await res.json()
    setMsg(JSON.stringify(j,null,2))
  }

  return (<div style={{background:'#0b1220', color:'#e6eef8', minHeight:'100vh', padding:20}}>
