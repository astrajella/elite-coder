...
export default function OrchestratorPage() {
  const [planText, setPlanText] = useState(JSON.stringify({
    plan_id: 'demo',
    steps: [{
      step_id: 's1',
      description: 'summarize',
      persona: 'summarizer',
      tool: 'summarize_tokens',
      payload: {
        context_blocks: ['example']
      },
    }, {
      step_id: 's2',
  ...
  useEffect(() => {
    loadRecharts().then(() => setChartsOk(rechartsImported))
  }, [])
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={async () => {
              ...
              const res = await fetch('/api/orchestrator/export/' + runId)
              if (res.ok) {
                const blob = await res.blob()
                const url = URL.createObjectURL(blob)
                const a = document.createElement('a')
                a.href = url
                a.download = `run_${runId}_metrics.csv`
            ...
            }} style={{ marginLeft: 12, padding: 8, background: '#3b82f6' }}>Download CSV</button>
          </div>
